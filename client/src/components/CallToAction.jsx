const CallToAction = ({saved, handleSave}) => {
  return (
    <div className='flex justify-between gap-4 items-center' onClick={handleSave}>
      ...
      <button className='flex gap-2 sm:gap-0 md:gap-4 font-bold items-center bg-[#0061E0] px-2 py-1 md:px-6 md:py-3 text-white hover:bg-[#09213f] ease-in-out duration-200 rounded'>
        <img src="/save.png" alt="" className={`w-3 h-3 md:w-5 md:h-5 ${saved && 'hidden'}`}/>
        {saved? 
          <h1 className="text-sm md:text-base">Your Favorite</h1> :
